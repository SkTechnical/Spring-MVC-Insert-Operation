...
public class StudentController {
	...
	public String updatePage(int id, Model model) {
		Optional<Student> upadte = studentRepository.findById(id);
		Student student = upadte.get();
         model.addAttribute("updateData", student);
		return "UpdateForm";
	}
	
	//to save the updated data and open view all data
	@RequestMapping("/saveUpdateData")
	public String saveUpdateData(Student student, Model model) {
		studentRepository.save(student);
		List<Student> view = studentRepository.findAll();
		model.addAttribute("data", view);
		return "ViewForm";
